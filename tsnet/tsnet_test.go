...
func TestResolveListenAddrUnspecified(t *testing.T) {
	tests := []struct {
		...
		{"empty host", "tcp", ":80", netip.Addr{}},
		{"ipv4 unspecified", "tcp", "0.0.0.0:80", netip.Addr{}},
		{"ipv6 unspecified", "tcp", "[::]:80", netip.Addr{}},
		{"specific ipv4", "tcp", "100.64.0.1:80", netip.MustParseAddr("100.64.0.1")},
		{"specific ipv6", "tcp6", "[fd7a:115c:a1e0::1]:80", netip.MustParseAddr("fd7a:115c:a1e0::1")},
	}
	for _, tt := range tests {
